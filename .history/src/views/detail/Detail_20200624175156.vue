      ...
      <div class="box">
        ...
        <div class="box1">
          ...
          <div v-if="nickname === ''" @click="collectGoods">
            收藏
            <van-rate v-model="value" icon="like" void-icon="like-o" color="red" void-color="#fff" :count="1"/>
          </div>
          <!--  未收藏  点击收藏-->
          ...
          <div v-else @click="cancelCollect">
            取消收藏
            <van-rate v-model="value" icon="like" void-icon="like-o" color="#fff" void-color="red" :count="1"/>
          </div>
        </div>
      </div>
      <div class="tab">
...
export default {
  ...
  methods: {
    ...
    // 收藏商品
    collectGoods() {
      if (this.isCollect !== 0 || this.nickname !== '') {
        this.$api
          .collection(this.obj)
          .then(res => {
            // 弹框提示
