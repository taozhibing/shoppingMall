  ...
  <div class="box">
    ...
    <van-cell-group>
      <van-field v-model="nickname" label="昵称">
        <template>{{this.nickname}}</template>
      </van-field>
    </van-cell-group>
    <van-cell-group>
      <van-field v-model="gender" label="性别">
        <template>{{this.gender}}</template>
      </van-field>
    </van-cell-group>
    <van-cell-group>
      <div>
...
export default {
  ...
  methods: {
    ...
    keep() {
      this.$api
        .saveUser({ gender:this.gender, year:this.year, month:this.month, day:this.day, id:this.id, nickname:this.nickname })
        .then(res => {
          this.$router.push("myself");
          console.log(123);
        })
