<template>
  <div>
    ...
    <div class="box">
      <div class="dv">{{addressComponent.city}}</div>
    </div>
    ...
    <div>
      <van-index-bar class="indexBar" :sticky="false" highlight-color="#AE853A">
        <van-index-anchor :index="item" v-for="(item,index) in msg" :key="index" />
        <van-cell v-for="(items,indexs) in values" :key="indexs" :title="items.name"/>
      </van-index-bar>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    let values = Object.values(this.city.data.cities)
    this.msg = keys;
    this.val = values;
    console.log(keys);
    console.log(values);
